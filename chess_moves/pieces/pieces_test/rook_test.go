...
func TestRook(t *testing.T) {
	const PossibleMoves = 28                 // Rook can have maximum possible 28 moves
	...
	piece := pieces.Rook {Piece: genericPiece}
	...
	for i:=0; i<len(moves); i++ {
		...
		if move[0] > 7 || move[0] < -7 {
			t.Errorf("Rook should not move than seven cell in one move")
		}
