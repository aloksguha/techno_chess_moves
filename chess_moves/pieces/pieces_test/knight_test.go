...
func TestKnight(t *testing.T) {
	const PossibleMoves = 8                 // Knight can have maximum possible 8 moves
	...
	piece := pieces.Knight {Piece: genericPiece}
	...
	for i:=0; i<len(moves); i++ {
		...
		if move[0] > 2 || move[0] < -2 {
			t.Errorf("Knight should not move than one cell in one move")
		}

		if move[1] > 2 || move[1] < -2 {
			t.Errorf("Knight should not move than one cell in one move")
		}
