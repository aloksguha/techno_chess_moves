...
func TestBishop(t *testing.T) {
	const PossibleMoves = 28                 // Bishop can have maximum possible 28 moves
	...
	piece := pieces.Bishop {Piece: genericPiece}
	...
	for i:=0; i<len(moves); i++ {
		...
		if move[0] > 7 || move[0] < -7 {
			t.Errorf("Bishop should not move than seven cell in one move")
		}
